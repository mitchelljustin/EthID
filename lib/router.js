...
router.route('/identity')
    .get((req, res, next) => {
        let identityValue = req.session.identityValue;
        if (!identityValue) {
            return next(new HttpError.BadRequest('need identity value'));
        }
        async.waterfall([
            done => {
                Identity
            ...
            (identities, done) => {
                ...
                let manager = ContractManager.get(identityType);
                if (!manager) {
                    return done(new HttpError.BadRequest(`no such identity type: ${identityType}`));
                }
                let contractModel = manager.contractModel.toJSON();
                let contract = manager.contract;
                let linkAddressTxData = contract.link.getData(identityValue);
                res.render('identity', {
                    title: `My ${identityType} Identity`,
                    profile: req.session.profile,
                    verifiedIds: verifiedIds,
                    unverifiedIds: unverifiedIds,
                    contract: contractModel,
                    linkAddressTxData: linkAddressTxData,
                });
                done();
            }
        ], (err) => {
