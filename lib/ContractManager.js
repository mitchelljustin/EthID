...
class ContractManager {
    ...
    setupExistingContract(callback) {
        ...
        this.loadAndCompileContract((err) => {
            ...
            let compiledCode = this.contractCompiled.EthereumID.code;
    ...
    loadAndCompileContract(done) {
        async.waterfall([
            ...
            (compiled, callback) => {
                this.contractCompiled = compiled;
    ...
    launchNewContract(callback) {
        let onContractCreated = (err, contract) => {
            ...
            var abiDefinition = this.contractCompiled.EthereumID.info.abiDefinition;
            Contract.create({
                identityType: this.identityType,
                ethereumAddress: contract.address,
                abiDefinition: JSON.stringify(abiDefinition),
                code: this.contractCompiled.EthereumID.code,
            }, (err, contractModel) => {
                if (err) {
                    return callback(err);
                }
